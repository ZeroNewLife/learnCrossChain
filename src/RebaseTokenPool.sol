...
//код в принципе работает если брать старуб версию ccip но в новых версиях поменялись 
//за счет чего функций pool tokenpool и.т.д отсутствует но при переносе версий все работает ок 
//так как у меня свежий компилятор работать в каждом классе под разными версиями убивает меня 
//желающие могут сами юзать но я пас потом как нибудь сделаю на новом стандарте
//CCT токен у меня в профиле уже лежит не велика потеря 
